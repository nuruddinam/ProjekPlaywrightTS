...
const AGENTQ_API_URL = 'https://backend-app.agentq.id';
...
export async function pushTestResultToAgentQ(testTitle: string, status: string, executionTime: number, errorDetails?: string) {
  if (!AGENTQ_TESTRUN_ID) {
    console.log('⚠️ AGENTQ_TESTRUN_ID not set, skipping AgentQ update');
    return;
  }

  // Mengambil angka ID di depan judul (Contoh: "1-" dari "1-User successfully...")
  ...
  await exportTestResult(tcId, {
    ...
    executionTime: executionTime / 1000, // Konversi ke detik
